...
def requestCodes(state):
    ...
    print(codesDict)
    with open(str(pathlib.Path().absolute()) + '\\weather_forecast\\state_codes\\%s.json' % state, "w+") as json_file:
...
def requestForecast(state='', city='', code=None):
    ...
    if os.path.isfile(str(pathlib.Path().absolute()) + '\\weather_forecast\\state_codes\\%s.json' % state):
        with open(str(pathlib.Path().absolute()) + '\\weather_forecast\\state_codes\\%s.json' % state, "r") as f:
            jdata = json.load(f)
    ...
    return Forecast(state, city, current, humidity, wind, high, low)
